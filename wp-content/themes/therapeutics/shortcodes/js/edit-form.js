        ...
        vc.atts.column_offset = {
            ...
            init: function(param, $field) {
                $('[data-column-offset="true"]', $field).each(function() {
                    ...
                    $field.data("vcColumnOffset", new VcColumnOffsetParam({
                        el: $this
                    }).render())
                })
            }
...
if (function($) {
        ...
        function filterSelection(selection, obj) {
            function finishImageProcessing(newAttachments) {
                ...
                }), obj._css_editor.$el.find(".vc_background-image .vc_image").html(_.template($("#vc_css-editor-image-block").html(), objects[0], _.extend({
                    variable: "img"
                }, template_options))), $(".media-modal").removeClass("processing-media")
            }
            var ids;
        ...
        function processImages(ids, callback) {
            $.ajax({
                ...
                $(".media-modal-close").click(), window.vc && window.vc.active_panel && window.i18nLocale && window.i18nLocale.error_while_saving_image_filtered && vc.active_panel.showMessage(window.i18nLocale.error_while_saving_image_filtered, "error"), window.console && window.console.error && window.console.error(response)
            }).always(function() {
                $(".media-modal").removeClass("processing-media")
            })
        }
        var removeOldDesignOptions, media = wp.media,
        ...
        VcCssEditor = vc.CssEditor = Backbone.View.extend({
            ...
            parseAtts: function(string) {
                ...
                border_regex = /(\d+\S*)\s+(\w+)\s+([\d\w#\(,]+)/, background_regex = /^([^\s]+)\s+url\(([^\)]+)\)([\d\w]+\s+[\d\w]+)?$/, background_size = !1, _.map(string.split(";"), function(val) {
                    var val_pos, border_split, background_split, val_s = val.split(/:\s/),
                        value = val_s[1] || "",
                        name = val_s[0] || "";
                    value && (value = value.trim()), name.match(new RegExp("^(" + this.layouts.join("|").replace("-", "\\-") + ")$")) && value ? (val_pos = value.split(/\s+/g), 1 === val_pos.length ? val_pos = [val_pos[0], val_pos[0], val_pos[0], val_pos[0]] : 2 === val_pos.length ? (val_pos[2] = val_pos[0], val_pos[3] = val_pos[1]) : 3 === val_pos.length && (val_pos[3] = val_pos[1]), _.each(this.positions, function(pos, key) {
            ...
            save: function() {
                ...
                return this.attrs = {}, _.each(this.layouts, function(type) {
                    this.getFields(type)
                }, this), this.getBorder(), _.isEmpty(this.attrs) || (string = ".vc_custom_" + Date.now() + "{" + _.reduce(this.attrs, function(memo, value, key) {
            ...
            getBorder: function() {
                ...
                style && color && this.attrs["border-width"] && this.attrs["border-width"].match(/^\d+\S+$/) ? (this.attrs.border = this.attrs["border-width"] + " " + style + " " + color, this.attrs["border-width"] = void 0, radius && (this.attrs["border-radius"] = radius)) : (_.each(sides, function(side) {
            ...
            getFields: function(type) {
                var data = [];
                return this.simplify ? this.getSimplifiedField(type) : (_.each(this.positions, function(pos) {
                    var val = this.$el.find("[data-name=" + type + "-" + pos + "]").val().replace(/\s+/, "");
                    val.match(/^-?\d*(\.\d+){0,1}(%|in|cm|mm|em|rem|ex|pt|pc|px|vw|vh|vmin|vmax)$/) || (val = isNaN(parseFloat(val)) ? "" : "" + parseFloat(val) + "px"), val.length && data.push({
                        name: pos,
                        val: val
                    })
                }, this), void _.each(data, function(attr) {
            ...
            parse: function(param) {
                ...
                return $field = this.content().find('input.wpb_vc_param_value[name="' + param.param_name + '"]'), css_editor = $field.data("vcFieldManager"), result = css_editor.save(), result && vc.edit_form_callbacks.push(removeOldDesignOptions), result
            },
            init: function(param, $field) {
                $("[data-css-editor=true]", this.content()).each(function() {
                    var $editor = $(this),
                        ...
                        value = $param.val();
                    value || (value = parseOldDesignOptions()), $param.data("vcFieldManager", new VcCssEditor({
                        el: $editor
                    }).render(value))
...
if (function($) {
            ...
            setParams: function() {
                ...
                if ($option.length) {
                    ...
                    _.each(data, function(value, key, list) {
                        var fieldManager, $field = $edit_form.find("[name=" + key + "].wpb_vc_param_value");
                        $field.length && ($field.unbind("change.vcParamsPreset"), fieldManager = $field.data("vcFieldManager"), fieldManager && fieldManager.render && fieldManager.render(value), fields.push($field), $field.val(value).trigger("change"))
        ...
        function InitGalleries() {
            $(".gallery_widget_attached_images_list", this.$view).unbind("click.removeImage").on("click.removeImage", "a.vc_icon-remove", function(e) {
                e.preventDefault();
        ...
        $tabs.length && ($(".wpb-edit-form").addClass("vc_with-tabs"), $tabs.find(".vc_edit-form-tab-control").removeClass("vc_active").eq(0).addClass("vc_active"), $tabs.find('[data-vc-ui-element="panel-edit-element-tab"]').removeClass("vc_active").eq(0).addClass("vc_active"), $tabs.find(".vc_edit-form-link").click(function(e) {
            var $this = $(this);
            e.preventDefault(), $tabs.find(".vc_active").removeClass("vc_active"), $this.parent().addClass("vc_active");
            var $target = $($this.attr("href")).addClass("vc_active");
            $target.parents(".vc_panel-body").scrollTop(0)
        }))
    }(window.jQuery), _.isUndefined(window.vc)) var vc = {
    atts: {}
};
! function($) {
    ...
    vc.atts.vc_grid_item = {
        init: function(param, $field) {
            !0 === vc_user_access().getState("grid_builder") || null === vc_user_access().getState("grid_builder") ? this.content().find('[data-vc-shortcode-param-name="' + param.param_name + '"] [data-vc-grid-element="value"]').change(function() {
                var value = $(this).val(),
